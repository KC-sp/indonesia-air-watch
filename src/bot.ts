...
const TRACKABLE_CITIES = [{ city: 'Bogor', state: 'West Java' }, { city: 'Bekasi', state: 'West Java' }, { city: 'Tangerang', state: 'Banten' }, { city: 'Depok', state: 'West Java' }, { city: 'Balikpapan', state: 'East Kalimantan' }];
export function createBot(config: Config & { TELEGRAM_BOT_TOKEN: string; OWNER_TELEGRAM_USER_ID: string }, air: AirService, news: OfficialNewsService, sources: SourceReviewService) {
  ...
  bot.command('setregion', (ctx) => ctx.reply('Choose a supported city to track. The list excludes national sample cities.', Markup.inlineKeyboard(TRACKABLE_CITIES.map((x) => [Markup.button.callback(`${x.city}, ${x.state}`, `add:${x.city}|${x.state}`)]))));
