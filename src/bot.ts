...
export function createBot(config: Config & { TELEGRAM_BOT_TOKEN: string; OWNER_TELEGRAM_USER_ID: string }, air: AirService, news: OfficialNewsService, sources: SourceReviewService) {
  ...
  bot.command('status', async (ctx) => { const tracked = await air.trackedStatus(); const sample = await air.sample(); await ctx.reply(hourlyMessage(sample.average, tracked.readings, undefined, tracked.unavailable)); });
  ...
  bot.command('regions', async (ctx) => { const locations = await air.trackedLocations(); await ctx.reply(locations.length ? locations.map((r) => `${r.city}${r.state ? `, ${r.state}` : ''}`).join('\n') : 'No tracked locations.'); });
  ...
  bot.command('news', async (ctx) => { const topic = ctx.message.text.replace(/^\/news(?:@\w+)?\s*/i, '').trim() || 'environment'; const items = await news.latest(topic); await ctx.reply(items.length ? items.map(formatNewsItem).join('\n\n') : 'No verified original official article was found.'); });
