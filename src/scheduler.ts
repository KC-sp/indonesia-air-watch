...
export function startHourlyScheduler(db: PrismaClient, send: () => Promise<void>) {
  const schedule = () => { const delay = 3_600_000 - (Date.now() % 3_600_000) + 50; setTimeout(async () => { try { await dispatchOnce(db, send); } catch (error) { logger.error(error, 'hourly dispatch failed'); } finally { schedule(); } }, delay); };
  schedule();
}
